...
import {
  getApplicationId,
  isResumeCase as getIsResumeCase,
} from "../../features/financial-details/utils/applicationStorage";
...
const initializeMockDatabase = () => {
  ...
  if (!getIsResumeCase()) {
    ...
    console.log(
      `Mock database initialized for RESUME case with application ID: ${getApplicationId()}`
    );
...
export const financialDetailsHandlers = [
  ...
  http.get("/api/overviewxapi/financial-commitments", async ({ request }) => {
    ...
    const commitments = getCommitmentsByApplicationId(
