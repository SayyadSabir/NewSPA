...
import {
  ...
  shouldShowField,
  FieldName,
} from "./form-fields/CommitmentFieldConfig";
import {
  TypeSelector,
  BalanceField,
  MonthlyPaymentField,
...
const CommitmentForm = forwardRef<CommitmentFormRef, CommitmentFormProps>(
  (props, ref) => {
    const {
      onAddCommitment,
      existingCommitment,
    ...
    useEffect(() => {
      ...
      fieldNames.forEach((fieldName) => {
        ...
        if (
          !shouldShowField(fieldName, fieldConfig, formValuesForConditionals)
        ) {
    ...
    const isFormComplete = React.useMemo(() => {
      ...
      const fieldValidators: Record<FieldName, (value: any) => boolean> = {
        ...
        termRemainingMonths: (val) =>
          val !== undefined && val !== null && val !== "",
        termRemainingYears: (val) =>
          val !== undefined && val !== null && val !== "",
