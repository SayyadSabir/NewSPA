...
const commitmentTypeOptions: { value: CommitmentType; label: string }[] = [
  ...
  {
    value: "guarantor_existing_borrowing",
    label: "Guarantor on existing borrowing",
  },
  {
    value: "guarantor_rental_agreement",
    label: "Guarantor on rental agreement",
  },
  {
    value: "hire_purchase",
    label: "Hire purchase (HP) or personal contract purchase (PCP)",
  },
  { value: "maintenance", label: "Maintenance for ex-partner or child" },
  { value: "overdraft", label: "Overdraft" },
  {
    value: "overdraft_secured",
    label: "Overdraft secured against investment portfolio",
  },
  { value: "personal_loan", label: "Personal loan or point-of-sale finance" },
  { value: "secured_personal_loan", label: "Secured personal loan" },
  {
    value: "shared_equity_loan",
    label: "Shared equity loan for another property",
  },
...
export const TypeSelector: React.FC<FormFieldProps> = ({
  register,
  watch,
  errors,
  setValue,
...
export const BalanceField: React.FC<FormFieldProps> = ({
  register,
  errors,
  fieldConfig,
  getValues,
...
export const MonthlyPaymentField: React.FC<FormFieldProps> = ({
  register,
  errors,
  fieldConfig,
}) => {
...
export const CompletionStatusSelector: React.FC<FormFieldProps> = ({
  register,
  watch,
  errors,
  setValue,
    ...
    <Grid item xs={12}>
      <FormControl component="fieldset" error={!!errors?.completionStatus}>
        <FormLabel component="legend">
          What will happen to this commitment by the time this mortgage
          completes?
        </FormLabel>
        <RadioGroup value={completionStatus} onChange={handleChange} row>
...
export const RepaymentAmountField: React.FC<FormFieldProps> = ({
  register,
  errors,
  fieldConfig,
}) => {
...
export const NotesField: React.FC<FormFieldProps> = ({
  register,
  errors,
  fieldConfig,
}) => {
...
export const IncludeInMortgageField: React.FC<FormFieldProps> = ({
  register,
  watch,
  errors,
  fieldConfig,
  ...
  return (
    <Grid item xs={12}>
      <FormControlLabel
        ...
        label={
          fieldConfig.labels.includeInMortgage ||
          "Include this commitment in the mortgage"
        }
      />
    </Grid>
  );
