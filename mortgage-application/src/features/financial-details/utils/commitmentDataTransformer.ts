...
export const transformToTermRemaining = (
  ...
  if (
    (isNaN(numericMonths) || numericMonths < 0) &&
    (isNaN(numericYears) || numericYears < 0)
  ) {
    return undefined;
  }
  ...
  return {
    ...
    "no-of-months":
      isNaN(numericMonths) || numericMonths < 0 ? 0 : numericMonths,
  };
};
...
export const transformCommitmentFormDataToApi = (
  ...
  Object.entries(FIELD_MAPPING_CONFIG.currencyFields).forEach(
    ([formField, apiField]) => {
  ...
  Object.entries(FIELD_MAPPING_CONFIG.simpleFields).forEach(
    ([formField, apiField]) => {
      const value = formData[formField];
  ...
  Object.entries(FIELD_MAPPING_CONFIG.compositeFields).forEach(
    ([compositeKey, config]) => {
...
export const transformApiDataToForm = (
  ...
  Object.entries(FIELD_MAPPING_CONFIG.currencyFields).forEach(
    ([formField, apiField]) => {
      const currencyAmount = (apiData as any)[apiField] as
        | CurrencyAmount
        | undefined;
  ...
  Object.entries(FIELD_MAPPING_CONFIG.compositeFields).forEach(
    ([compositeKey, config]) => {
      if (compositeKey === "termRemaining") {
        const termRemaining = (apiData as any)[config.apiField] as
          | TermRemaining
          | undefined;
        if (termRemaining) {
