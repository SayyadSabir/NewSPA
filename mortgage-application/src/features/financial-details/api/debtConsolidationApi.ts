...
export const debtConsolidationApi = createApi({
  ...
  endpoints: (builder) => ({
    ...
    saveDebtConsolidation: builder.mutation<DebtConsolidationResponse, DebtConsolidationRequest>({
      query: (request) => ({
      ...
      onQueryStarted: async (request, { dispatch, queryFulfilled }) => {
        ...
        try {
          ...
          dispatch(setError(error.error?.data?.message || 'Failed to save debt consolidation data'));
        }
      },
    }),
  }),
